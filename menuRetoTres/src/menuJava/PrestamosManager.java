...
public class PrestamosManager {
    ...
    private static void menuLibros(Scanner sc) {
        ...
        Io.Sop("4. Consultar libro por nombre");
        Io.SOP("0. Volver");
    ...
    private static void altaLibro(Scanner sc) {
        try {
            ...
            String sqlLibro = "INSERT INTO libros (titulo, isbn, genero) VALUES (?, ?, ?)";
    ...
    private static void modificarLibro(Scanner sc) {
        try {
            ...
            String sqlLibro = "UPDATE libros SET titulo = ?, isbn = ?, genero = ? WHERE id_libro = ?";
    ...
    private static void consultarLibroPorNombre(Scanner sc){
        Io.sop("Introduce el nombre del libro a consultar: ");
        ...
        try(PreparedStatement stmt=conn.prepareStatement(sql)){
            ...
            try(ResultSet rs=stmt.executeQuery()){
                while (rs.next()) {
                    Io.sop("ID: "+rs.getInt("id_libro")+" | Título: "+rs.getString("titulo")+
                    " | ISBN: "+rs.getLong("isbn")+" | Género: "+rs.getString("genero"));
                }
            }
        } catch (SQLException e){
            Io.sop("Error al consultar el libro: "+e.getMessage());
        }
    }
    ...
    private static void buscarLibrosPorAutor(Scanner sc) {
        try {
            ...
            String sql = "select a.id_autor, a.nombre as nombre_autor, a.apellidos as apellidos_autor, l.id_libro, l.titulo from autores a join libauto la on a.id_autor=la.id_autor join libros l on l.id_libro=la.id_libro where a.nombre like ?";
            ...
            try (PreparedStatement stmt = conn.prepareStatement(sql)) {
                ...
                while (rs.next()) {
                    ...
                    if (!mostrarAutor) {
                        ...
                        String apellidos= rs.getString("apellidos_autor");
                        int idAutor=rs.getInt("id_autor");
                        Io.SOP("\nAutor: "+nombre+ " "+ apellidos+", ID: "+idAutor+"\n");
                        mostrarAutor=true;
                    }
                    //Después se muestran los datos de todos los libros
                    int idLibro =rs.getInt("id_libro");
                    String titulo=rs.getString("titulo");
                    Io.sop("Libro: "+titulo+", ID: "+idLibro);
                }
            }
        } catch (Exception e) {
            Io.sop("Error. Verifica que el nombre del autor sea correcto.");
        }
    }
